...
import {IconButton,Avatar,Box,CloseButton,Flex,HStack,VStack,Icon,useColorModeValue,Text,Drawer,DrawerContent,
  useDisclosure,BoxProps,FlexProps,Menu,MenuButton,MenuDivider,MenuItem,MenuList,Link,} from '@chakra-ui/react'
import {FiHome,FiMenu,FiBell,FiChevronDown,} from 'react-icons/fi'
import { Outlet, Link as RouterLink } from 'react-router-dom'
import { HiOutlineViewColumns } from 'react-icons/hi2'
import { BsGrid3X3 } from 'react-icons/bs'

interface MobileProps extends FlexProps {onOpen: () => void}
...
const NavItem = ({to, icon, children, ...rest }) => {
  return (
    <Link as={RouterLink} to={to} style={{ textDecoration: 'none' }} _focus={{ boxShadow: 'none' }}>
      <Flex align="center" p="4" mx="4" borderRadius="lg" role="group" cursor="pointer"  _hover={{
          bg: 'cyan.400', color: 'white',}}  {...rest}>
        {icon && (
...
const MobileNav = ({ onOpen, ...rest }: MobileProps) => {
  return (
    <Flex ml={{ base: 0, md: 60 }} px={{ base: 4, md: 4 }} height="20" alignItems="center"
      bg={useColorModeValue('white', 'gray.900')} borderBottomWidth="1px"
      borderBottomColor={useColorModeValue('gray.200', 'gray.700')} justifyContent={{ base: 'space-between',
         md: 'flex-end' }}   {...rest}>
